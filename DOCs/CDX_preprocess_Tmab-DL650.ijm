/**
Multi-Slice TIFF Image Processor for ImageJ (Version 2)
------------------------------------------------------
This macro script processes a multi-slice TIFF image by splitting the image into smaller stacks of slices. On each stack, it applies background subtraction and max intensity projection. The processed stacks are saved into a new directory.

This version uses a larger rolling ball radius for background subtraction and processes images from a specific directory.
...
originalFileName = "CDX3c_DL650_full.tif";
...
for (i = 0; i < totalSlices; i += 10) {
    ...
    // Apply background subtraction with a larger rolling ball radius
